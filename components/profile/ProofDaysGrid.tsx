...
import { proofsTileIsMissing, type ProofsGridItem } from "@/lib/proofs-grid";
...
export function proofDaysFromItems(items: readonly ProofsGridItem[]): ProofDay[] {
  const days = proofDaysFromPhotos(items.map((i) => ({ dateKey: i.dateKey, uri: i.uri, shared: i.shared })));
  return days.map((day) => {
    const onDay = items.filter((i) => i.dateKey === day.dateKey);
    const live = onDay.find((i) => !proofsTileIsMissing(i));
    return {
      ...day,
      coverUri: live?.uri ?? day.coverUri,
      missing: onDay.length > 0 && onDay.every((i) => proofsTileIsMissing(i)),
    };
  });
}
