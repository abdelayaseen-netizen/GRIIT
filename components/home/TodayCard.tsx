/**
 * Presentational Today card (Chunk J). Ported: this branch does not
 * include feat/today-source-of-truth. Static model only; no query.
 */
...
function TaskRow({
  ...
  const inner = (
    <>
      ...
      {task.done ? null : <ChevronRight size={DOT} color={DS_V3.color.textSecondary} />}
    </>
  );
      ...
      <View style={styles.stack}>
        ...
        {model.groups.map((group) => (
          <View key={group.active_challenge_id} style={styles.group}>
            {model.labelled ? <Text style={styles.groupLabel}>{group.challenge_name}</Text> : null}
            {group.tasks.map((task) => (
              <TaskRow
                key={task.id}
                task={task}
                onPress={
                  task.done ? undefined : () => onTask?.(group.active_challenge_id, task.id)
                }
              />
            ))}
          </View>
