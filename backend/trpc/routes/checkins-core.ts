...
import { type ChallengeTaskConfig, type ChallengeTaskRowRaw, getTaskVerification, isTaskRequired } from "../../lib/challenge-tasks";
...
export const checkinsCoreProcedures = {
  complete: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (requireLocation) {
        ...
        if (input.location_latitude == null || input.location_longitude == null) throw new TRPCError({ code: "BAD_REQUEST", message: "This task requires location verification. Please enable location services." });
        const distance = haversineDistance(tLat, tLon, input.location_latitude, input.location_longitude);
        if (distance > radius) {
          throw new TRPCError({ code: "BAD_REQUEST", message: `You need to be within ${radius}m of ${task?.location_name ?? cfg.location_name ?? "the required location"}. You are ${Math.round(distance)}m away.` });
        }
      }
