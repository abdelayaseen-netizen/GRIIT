...
export const feedRouter = createTRPCRouter({
  ...
  getPost: protectedProcedure.input(z.object({ eventId: z.string().uuid() })).query(async ({ ctx, input }) => {
    const server = getSupabaseServer() ?? ctx.supabase;
    const viewerId = ctx.userId;
    const { data: raw, error: evErr } = await server.from("activity_events").select("id, user_id, event_type, challenge_id, metadata, created_at, shared, share_state").eq("id", input.eventId).maybeSingle();
    if (evErr) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: evErr.message });
    if (!raw) throw new TRPCError({ code: "NOT_FOUND", message: "Post not found" });
    const ev = raw as EvRow & { share_state?: string };
    if (ev.share_state !== "shared" && ev.shared === false && ev.user_id !== viewerId) throw new TRPCError({ code: "NOT_FOUND", message: "Post not found" });
  ...
  shareCompletion: protectedProcedure.input(z.object({ challengeId: z.string().uuid(), caption: z.string().max(120).optional(), proofPhotoUrl: z.string().url().optional() })).mutation(async ({ input, ctx }) => {
    ...
    if (existingEvent) {
      ...
      const { data: inserted, error: insErr } = await activityEventsWriter.from("activity_events").insert({ user_id: ctx.userId, event_type: "task_completed", challenge_id: input.challengeId, shared: true, share_state: "shared", metadata: insertMeta } as never).select("id, metadata").single();
      if (insErr || !inserted) {
