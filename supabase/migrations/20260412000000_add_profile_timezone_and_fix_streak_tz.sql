-- IANA tz name (e.g. America/New_York). Used for date_key in secure_day and backend helpers; default UTC.
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS timezone TEXT DEFAULT 'UTC';

-- Replace secure_day: date_key follows the user's local calendar day (profiles.timezone), not UTC.
...
COMMENT ON FUNCTION public.secure_day(uuid) IS 'Atomically secure the day. Reads required from challenge_tasks.config. date_key uses profiles.timezone.';
