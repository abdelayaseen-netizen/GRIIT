-- Fix secure_day RPC: challenge_tasks.required does NOT exist as a top-level column
-- on the live database. required lives in config JSONB. The old RPC queried
-- ct.required = true which throws "column does not exist" at runtime.
-- Replace with (ct.config->>'required')::boolean IS NOT FALSE to match isTaskRequired()
-- in challenge-tasks.ts (treats missing/null as required=true, only false opt-out).
...
BEGIN
  ...
  -- Idempotent: already secured today — return current streak, no double-count
  ...
  -- Required tasks: read from config JSONB (required IS NOT FALSE means required).
  -- Matches isTaskRequired() in challenge-tasks.ts.
  SELECT ARRAY_AGG(ct.id) INTO v_required_ids
  FROM challenge_tasks ct
  WHERE ct.challenge_id = v_challenge_id
    AND (ct.config->>'required')::boolean IS NOT FALSE;

  SELECT ARRAY_AGG(ci.task_id) INTO v_completed_ids
  FROM check_ins ci
  WHERE ci.active_challenge_id = p_active_challenge_id
  ...
  FROM day_secures
  WHERE user_id = v_uid
    AND date_key >= ((v_date_key::date) - interval '6 days')::date::text
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    last_stands_available = CASE WHEN v_earn_stand
      THEN LEAST(2, COALESCE(streaks.last_stands_available, 0) + 1)
      ELSE COALESCE(streaks.last_stands_available, 0) END,
    last_stands_used_total = CASE WHEN v_earn_stand
      THEN COALESCE(streaks.last_stands_used_total, 0) + 1
      ELSE COALESCE(streaks.last_stands_used_total, 0) END;

  UPDATE active_challenges
  SET current_day = COALESCE(current_day, 0) + 1, progress_percent = 100
  ...
  UPDATE profiles SET total_days_secured = v_total_days, tier = v_tier, updated_at = now()
  WHERE user_id = v_uid;
...
COMMENT ON FUNCTION public.secure_day(uuid) IS
  'Atomically secure the day: day_secures, streaks, active_challenges, profiles. '
  'Idempotent per (user, day). Reads required tasks from config JSONB. Call with user JWT.';
