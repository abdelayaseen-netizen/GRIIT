-- secure_day: read required from challenge_tasks.config (real schema has task_type + config, no flat required column).
...
COMMENT ON FUNCTION public.secure_day(uuid) IS 'Atomically secure the day. Reads required from challenge_tasks.config.';
